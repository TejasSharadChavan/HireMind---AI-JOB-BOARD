    ...
    >
      <CardHeader>
        <div className="flex gap-4">
          ...
          {jobListing.postedAt != null && (
            <div className="text-sm font-medium text-primary max-md:hidden">
              <Suspense fallback={jobListing.postedAt.toLocaleDateString()}>
                <DaysSincePosting postedAt={jobListing.postedAt} />
              </Suspense>
            </div>
...
async function getJobListings(
  ...
  function toStringArray(input?: string | string[] | null): string[] {
    if (!input) return [];
    return Array.isArray(input)
      ? input.filter(Boolean)
      : [input].filter(Boolean);
  }
  const jobIds = toStringArray(searchParams.jobIds);
