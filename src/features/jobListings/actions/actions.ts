...
export async function updateJobListing(
  ...
  if (!success) {
    return {
      error: true,
      message: "There is an error updating your job listing",
    };
  }
...
export async function toggleJobListingStatus(id: string) {
  ...
  if (
    !(await hasOrgUserPermission("org:job_listing:change_status")) ||
...
export async function deleteJobListing(id: string) {
  const error = {
    error: true,
    message: "You don't have permission to delete this job listings",
  };
  const { orgId } = await getCurrentOrganization();
  if (orgId == null) return error;
