...
export const prepareDailyUserJobListingNotifications = inngest.createFunction(
  ...
  async ({ step, event }) => {
    const getUsers = await step.run("get-users", async () => {
    ...
    const getJobListings = await step.run(
...
export const sendDailyUserJobListingEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!matchingJobListings || matchingJobListings.length === 0) return;

    await step.run("send-email", async () => {
      await resend.emails.send({
        from: "Job Board <onboarding@resend.dev>",
        to: "tejaschavan685@gmail.com",
